...
class ToDoListViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        items = itemManager.getItemsFromRealm(category: self.selectedCategory)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView.cellForRow(at: indexPath)?.accessoryType == .checkmark {
            ...
            itemManager.updateItemFromRealm(item: selectItem, done: false, category: self.selectedCategory)
            
        } else {
            tableView.cellForRow(at: indexPath)?.accessoryType = .checkmark
            let selectItem = items[indexPath.row]
            itemManager.updateItemFromRealm(item: selectItem, done: true, category: self.selectedCategory)
        }
    ...
    @IBAction func addButtonTapped(_ sender: UIBarButtonItem) {
        ...
        alert.addAction(UIAlertAction(title: "추가", style: .default, handler: { action in
            self.itemManager.saveItemInRealm(toDoText: textField.text, done: false, category: self.selectedCategory) {
                self.items = self.itemManager.getItemsFromRealm(category: self.selectedCategory)
            }
        }))
...
extension ToDoListViewController: UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        let itemList = itemManager.getItemsFromRealm(category: self.selectedCategory)
